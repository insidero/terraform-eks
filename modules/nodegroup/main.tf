...
resource "aws_eks_node_group" "default" {
  count           = local.enabled && ! var.create_before_destroy ? 1 : 0
  node_group_name = "${var.label.id}-worker-nodegroup"
...
resource "aws_eks_node_group" "cbd" {
  count           = local.enabled && var.create_before_destroy ? 1 : 0
  node_group_name = format("%v%v%v", "${var.label.id}-worker-nodegroup", var.label.delimiter, join("", random_pet.cbd.*.id))
